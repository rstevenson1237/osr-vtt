...
| IN-216 | Playwright has no suite-wide timeout or failure cap, so a systemic break costs hours instead of failing fast | **Simple** | **Open** | Classification approved (user, 2026-09-23), proposed as WI-199 (Batch 4), awaiting gate — reported by the user, 2026-09-23: `apps/web/playwright.config.ts` sets only a per-test `timeout: 180_000`; add `globalTimeout` and `maxFailures`; suggested model `haiku` |
| IN-217 | `pnpm verify:all` shows only the last 40 lines of a failing step, which hid the failing test | **Simple** | **Open** | Classification approved (user, 2026-09-23), proposed as WI-200 (Batch 4), awaiting gate — reported by the user, 2026-09-23: `scripts/verify.mjs` `TAIL_LINES = 40`; `pnpm test:all:emulators` shows full output. Surface the failing test names (or full output on failure); suggested model `haiku` |
