...
function countHoles(mp: MultiPoly): number {
  let h = 0;
...
export function commitCarve(
  ...
  const combined = mode === 'add' ? backend.union(floor, strokes) : backend.difference(floor, strokes);
