# Decision Log — Vector Map System

Ratified decisions, folded into [`VectorMapSystem_Spec.md`](./VectorMapSystem_Spec.md).
Each row states the ruling and its source (**user** = product direction
2026-07-19; **rec** = Claude Code recommendation accepted into the working spec
pending objection).

> This log originated under `poc/vector-floor/DECISIONS.md` during the POC that
> designed and shipped the Vector Map System (WI-A–WI-D, complete). The
> scaffold — including its `REVIEW.md` conflict analysis and `FINDINGS.md` POC
> results — has since been deleted; the "Findings disposition" table below
> preserves what resolved each review conflict, and a condensed version of
> `FINDINGS.md`'s evidence lives in the
> [POC evidence appendix](#poc-evidence-historical) at the end of this file.
...
Why (full analysis in the ["Select-tool identity finding"](#poc-evidence-historical)
below):
...
**Governing premise (user):** _if the POC is accepted, Firebase is wiped and the
pure vector system rolls out at WI-D._ There is therefore **no stored data to
...
temporary _code_ crutch to be deleted at WI-D, not a durable feature. WI-B was
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **B1** | **Vector-wall collection name.** SPEC §3.1 stores explicit walls at `maps/{mapId}/walls/{wallId}`, colliding with the cellular `MapWall` collection at that path.                                                                                       | **Keep `wallSegments` through WI-C.** The collision is real even under a wipe because cellular _code_ runs until WI-D (`mapWallConverter` and `vectorWallSegmentConverter` would both bind `walls` and choke on each other's docs). `floorRegions`/`doors` keep the spec names.                                                                                                                                                                                                                              | **Rename `wallSegments` → `walls`** everywhere. Single source of truth is `VECTOR_MAP_COLLECTIONS`; the rules file is the one hard-coded duplicate to audit.                                                                                                                                                                              |
| **B2** | **Per-map model discriminator.** Should `GameMap` carry `mapModel: 'cellular' \| 'vector'`?                                                                                                                                                             | **No discriminator.** A per-doc flag only earns its keep if both models coexist in one live deployment, which the wipe rules out; adding it re-imports the migration/dual-read scaffolding SPEC §2.3 / C2–C3 already rejected, as _permanent_ debt. Cutover is a **deploy-time event**, gated by the room-level `schemaVersion` bump (pre-wipe rooms trip the §2.3 "unsupported schema" error). During WI-C/WI-D dev, gate the two code paths with **one build/config feature flag**, never a per-map field. | **Delete the cellular store methods, converters, schemas, rules, and collections outright** — no dormant code (dormant cellular code is exactly what would force a discriminator back). ⚠️ **Product ack:** safe _only_ because no old cellular map need be openable after launch. If that changes, B2 flips → discriminator + dual-read. |
...
| **B5** | **`commitFloorRegions` batch size.**                                                                                                                                                                                                                    | **Single atomic batch; treat "≤500 ops per floor commit" as an invariant** (guaranteed by §8.2's ~8-region cap; bytes fine too — 500 × ~11 KiB ≈ 5.5 MiB < ~10 MiB batch cap). Single-batch is _required_ for atomicity: a partially-committed merge/split would corrupt the floor, so chunking merge/split is disallowed. Chunk to `FIRESTORE_BATCH_LIMIT` **only** for a future _non-atomic bulk_ op (e.g. "clear floor"), never for merge/split.                                                          | None.                                                                                                                                                                                                                                                                                                                                     |
...
1. ~~**Freehand `Drawing`/annotation layer**~~ ✅ **Resolved (2026-07-20).**
   ...
   the other drag tools), not the shared `MapToolbar`; only its _render_ target
   is the shared overlay layer. Text-annotation _authoring_ is not exposed yet
...
2. ~~**Peer cursors + pings**~~ ✅ **Resolved (2026-07-20).** `vector-engine.ts`
   ...
   the click point. (Peer _token-drag_ frames stay at parity — `subscribeDrag`
...
> **⚠️ Map-edit permissions.** The old cellular map hid _all_ editing tools from
...
> change); only **`add-creature`** is GM-gated. If players should _not_ be able
...
<a name="poc-evidence-historical"></a>

## POC evidence (historical)

Condensed from the original `poc/vector-floor/FINDINGS.md` (POC gate evidence,
2026-07-19) and `REVIEW.md` (the codebase-grounded conflict analysis that
produced the "Findings disposition" table above). Both files — along with the
disposable in-memory sandbox they evaluated — have been deleted now that their
conclusions are folded in here and their code has fully graduated to
production. Kept for anyone tracing _why_ a decision was made, not as a live
reference.

### What the POC build demonstrated (WI-1 gate)

Proven against the real geometry library (`polygon-clipping`), in lattice
units, pixels only at the render boundary (SPEC §2.0):

- **Five floor primitives** (Room, Corridor, Path, Polygon, Regular n-gon) with
  a per-point **snap / half / free** toggle (Alt = temporary freeform) — one
  shared point-stream → shape → pipeline path, five collectors (SPEC §2.5).
- **Interior rock-carve holes** (SPEC §2.4): subtract mode cuts a hole; a full
  bisection **splits one region into two automatically** — falls out of the
  boolean op with no special-casing.
- **Unified wall model** (SPEC §3.1): perimeter derived from the polygon
  boundary + explicit `Wall`-tool segments, all `blocksSight`/`blocksMovement`
  tagged.
- **Doors as floating-layer overlay objects** (SPEC §3.2/§3.4) reconciled at
  **build time** (SPEC §3.3): open door clips a real gap in sight, closed door
  blocks.
- **Snapshot undo/redo** across merge/split (SPEC §8.5).
...
Verification: 11/11 geometry assertions passed, headless UI smoke passed with
zero console errors.

**Select-tool identity finding** (fed the Model A decision above): editing
directly on the baked union boundary resolves the common cases cleanly —
edge-drag on a rectangle keeps it rectangular, vertex-drag reshapes freely.
What a baked union _cannot_ do is primitive-specific behavior that needs
retained identity (an n-gon "remembering" its side count/radius for uniform
rescale, or re-snapping a freely-dragged rectangle back to perfect
rectangularity). That tradeoff is what Model A knowingly gave up in exchange
for bounded, self-pruning storage — see [Model A](#model-a) above.

### §8 benchmark answers (evidence-backed)

- **§8.1 Library choice.** `polygon-clipping` (pure JS) handles union /
  difference / holes / split-on-bisection correctly at trivial bundle cost
  (45 KB minified / 15 KB gzip), but has **no offsetting and no simplify** —
  confirmed the hard gap that drove the `bufferPolyline` + Douglas-Peucker
  stand-in (see `packages/shared/src/map/vector/OFFSET-SPIKE.md` for the
  follow-up measurement that kept it).
- **§8.2 Vertex/document-size ceiling.** Stress test: 300 random carve +
  interior-rock ops on one map. Worst-case single region ≈ 11 KiB even with
  simplification off — ~90× under Firestore's 1 MiB limit. Size-driven spatial
  splitting is not needed at realistic scale.
- **§8.3 Simplification tolerance.** 0.10–0.15 (lattice units) read visually
  clean on both grid-aligned and organic shapes while cutting ~25–35% of
  vertices; 0.25 started visibly rounding grid corners.
- **§8.4 Performance.** ~0.6–1.1 ms average per carve/boolean/simplify commit
  across 300 ops — comfortable, no per-frame point-in-polygon anywhere.
- **§8.5 Undo granularity.** Snapshot-based undo (`structuredClone` of
  `{floor, walls, doors}`) handled merge/split cleanly; delta undo isn't
  well-defined across a split.

### Original review conflicts (REVIEW.md), for context

The "Findings disposition" table earlier in this file (C1–C5, M1–M8, R1–R2)
resolves each of these; see that table for the ruling and where it landed in
the spec. In brief, the review's job was to surface: a coordinate-space
mismatch between lattice and pixel units (C1); a conflict between "no
migration" and the Master Plan's portability hard rule (C2); a room-vs-map
`schemaVersion` scoping question mooted by the new-session-only assumption
(C3); the loss of zero-storage emergent fog once fog was removed (C4); and
scattered door/wall homes that made organic-boundary doors awkward (C5) — plus
eight smaller gaps (M1–M8) and two items later confirmed as non-issues
(R1–R2).
