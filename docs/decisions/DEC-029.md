...
> **Superseded in part by DEC-102** (answered (b), user, 2026-09-18): IN-145 asked whether the
> hook's durable state should move off a tracked file, or whether the guard was still needed at
> all. The guard, its trigger surface and the 15-minute window below all stand; the state moves
> to a gitignored `.claude/status.local`, so `PLAN.md`'s mtime is no longer what is read. The
> `PreToolUse` count stays at three. WI-150 lands it; this entry is named and reconsidered
> there, not overwritten here (RULE-019).
