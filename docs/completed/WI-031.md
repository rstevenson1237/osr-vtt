...
- `apps/web/src/lib/components/CharacterDock.svelte` — Added `selectedToken` derived
  state, `handleResizeToken` calling `store.resizeToken()` directly, and token scale
  control UI in the Map defaults section. Three testids moved verbatim:
  `token-scale-control`, `token-scale-slider`, `token-scale-value`. Disabled when `readOnly`.
...
- **Always call store directly, not through map controller.** `mapCtrl.onResizeToken` would
  resize the map-selected token rather than the character's own; always calling
  `store.resizeToken()` with the character's token ID is correct regardless of map selection.
