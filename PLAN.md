...
| --- | ------------ | ---- | ---- | ----- | ----- | ------ | ---- |
| WI-139 | **The icon record.** Six redraws (`encounter`, `session`, `room`, `corridor`, `ngon`, `polygon`); four hex/map glyphs (`road`, `river`, `terrain`, `hex`); 37 new chrome-verb and action glyphs; the `IconId` union grows 39 → 78; three render stops as `--icon`/`--icon-sm` in `theme/sizing.css` and `Icon.svelte`'s default. Draws nothing into a panel — every new id is unused until WI-140/141. | SPEC-051 §§2, 5, 6 | IN-136, IN-137, IN-138 (+ the glyphs IN-134, IN-139 consume) | claude-code | `sonnet` | M | ✅ **Gate cleared — user, 2026-09-18.** DEC-098's three stops (16/20/24 by pointer coarseness) approved as put. Execution is a fresh session. |
...
**WI-131 has now run and closed (2026-09-14)** — `docs/completed/WI-131.md` — SPEC-049: the
Measure tool resolves both drag ends to hexes on a hex map (`hexAt`, the same `pixelToAxial`
Select's own click uses) and reports `hexMap.axialDistance` instead of the square-lattice
`hypot` `toLatticeRaw` fed it, fixing the live RULE-006 breach. The Grid & measurement label
reads "Per hex" on a hex map (`isHexMap`), and `createDefaultGameMap` seeds
`{ perSquare: 6, unit: 'miles' }` for a freshly created hex map only — **no backfill, no
migration** (DEC-093 amended, user 2026-09-11): existing hex maps keep `{10, feet}`.
...
**WI-128 has now run and closed (2026-09-14)** — `docs/completed/WI-128.md` — no code change: the
gate's premise was already stale. WI-123 (2026-09-11) had applied `paintableHexContentsCatalog()`
to `MapToolbar`'s `HEX_SYMBOL_KINDS` as well as `HexTilePanel`'s `CONTENTS`, three days before
IN-122 was raised checking WI-121's own fix. `pnpm verify` green, unchanged.

**WI-126 has now run and closed (2026-09-13)** — `docs/completed/WI-126.md` — the Road/
River gesture is previewed while it is drawn. `renderHexLinePreview` on the `tools` layer,
sharing `strokeHexLine` with `renderHexLines` and `hexLinePointFor` with the vertex click,
so the ghost cannot drift from what the double-click will commit. Nothing is written
anywhere. **This leaves `renderHexLines` with a diff WI-133 must read before it smooths a
river there** — the caution the ordering note below already carries, now live.
...
several of these items sit in the same files. RULE-016 still means **one session, one work
item**, and each item's model target in the table is binding. Three are `opus` (WI-126, WI-130,
WI-133 — all render-pass work, and only for that reason); the rest are `sonnet` or `haiku`, and
running one of those on `opus` spends the month's allocation several times over for no gain.

**Ordering is preference, with two things worth knowing.** Nothing here blocks anything else.
But **WI-126 and WI-133 both live in `renderHexLines`** — the preview and the river's smoothing
— so whichever runs second reads the other's diff first rather than assuming the file it was
specified against. And **WI-129 also touches the Road/River toolbar** that WI-126 previews from,
which is a lighter version of the same caution. Everything else is disjoint.

**The original two-round record follows.** **WI-124 – WI-128 were gate-cleared first** and are the schedulable half of
the hex-crawl playtest batch below. **Three more items — WI-129, WI-130, WI-131 — are proposed
and awaiting disposition**: each was Deceptive at triage and each has since been unblocked by
an answer, not by a reclassification. WI-130 by **DEC-092** (b, the exact hex as the clip),
WI-131 by **DEC-093** (a, with a backfill — which is what makes it a migration), and WI-129 by
the user's own reading of "colour defaults from the selection" (no colour control; the shade is
...
`docs/completed/WI-114.md` — so `usedGroupLetters` reads `Token.letter` instead of parsing
