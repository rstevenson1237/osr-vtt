...
**Provenance is answered (user, 2026-09-02), and WI-101 is unblocked.** The 37 `.svg` files
have **no third-party source**: the project owner states they were generated by Claude in a
separate session, for a separate project of their own, and supplied here. Nothing was cloned,
traced or copied from a licensed pack, which is the risk SPEC-003 §5's licence discipline
exists to contain. WI-101 records exactly that in `ATTRIBUTION.md` — attributing them as the
owner's own generated assets, not as third-party art — and creating that file is where the
pack's entry goes. Until then the files stay parked, inert and referenced by nothing, at
`docs/intake/hex-symbols/`; moving one into `apps/web/public/` is part of WI-101, never a side
change (RULE-015).

**This does not close IN-078 by itself.** WI-101 creates `ATTRIBUTION.md` and writes the hex
pack's entry; IN-078's full scope is the file SPEC-003 §5 cites, which also owes an entry for
