 ...
 *     + explicit walls
 *     + imported walls
 *   → door reconciliation at BUILD time (SPEC §3.3):
 ...
 *
 * Everything is rebuilt once per render (build-once, probe-many) — no
 * per-frame-per-cell cost.
 */
 ...
 * holes) of the floor union. This REPLACES the old bit-walker `derivePerimeter`
 * — the boundary is the polygon boundary itself (SPEC §3.1, `source:'perimeter'`).
 * Perimeter defaults to blocking both sight and movement (SPEC §3.1, M8).
 */
...
/** Is point p on the infinite line through a→b (within a doors-only tolerance)?
 * ~1/12 cell: a door only has to be "near" the wall, not exactly on it. */
 ...
 * perimeter + sight-blocking explicit/imported, reconciled against the doors.
 */
export function buildSightSegments(
  floor: MultiPoly,
  explicit: Segment[],
  doors: Door[],
): Segment[] {
  const base = [...perimeterSegments(floor), ...explicit.filter((s) => s.blocksSight)];
  return reconcileDoors(base, doors);
}

/**
 * Movement-blocking segments (SPEC §3.1 passage model, REVIEW M8): perimeter +
 * movement-blocking explicit, reconciled against the doors (open doors carve a
 * passage, closed/barred doors block it). The polygon-boundary replacement for
 * the old `isEdgeBlocked` passage path.
 */
export function buildMovementSegments(
  ...
  const base = [...perimeterSegments(floor), ...explicit.filter((s) => s.blocksMovement)];
  return reconcileDoors(base, doors);
}
 ...
 * shadow. Proof that door reconciliation actually drives LoS.
 */
