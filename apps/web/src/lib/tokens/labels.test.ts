...
describe('tokenRingColor', () => {
  ...
  it('is white when the token is owned by the viewer, even unselected', () => {
    ...
    expect(tokenRingColor(token, groups, [], 'alice')).toBe('#ffffff');
  });
  ...
  it('falls back to the group color (via Group.memberTokenIds) when grouped but not selected/owned', () => {
    ...
    expect(tokenRingColor(token, groups, [], 'bob')).toBe(groupColor('goblins'));
  });

  it('is black when ungrouped and not selected/owned', () => {
    const token = mapToken();
    expect(tokenRingColor(token, [], [], 'bob')).toBe('#000000');
  });
  ...
  it('is white for every member of a multi-token selection (SPEC-056 §3)', () => {
    const t1 = mapToken({ id: 't1' });
    const t2 = mapToken({ id: 't2' });
    const selected = ['t1', 't2'];
    expect(tokenRingColor(t1, [], selected, 'bob')).toBe('#ffffff');
    expect(tokenRingColor(t2, [], selected, 'bob')).toBe('#ffffff');
  });
});
